...
class CleanupManager:
    ...
    def cleanup_now(self):
        ...
        try:
            ...
            cleaned_temp, freed_temp = self._clean_directory(self.temp_dir, self.temp_max_age, "временных файлов")
            cleaned += cleaned_temp
            freed_space += freed_temp

            # Очистка кэша
            cleaned_cache, freed_cache = self._clean_directory(self.cache_dir, self.cache_max_age, "кэша")
            ...
            if cleaned > 0:
                logger.success(f"✅ Очищено {cleaned} файлов, " f"освобождено {freed_space / (1024*1024):.1f} MB")
            else:
