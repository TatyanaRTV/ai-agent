...
class ElenaAgent:
    ...
    def _init_components(self):
        ...
        try:
            ...
            self.components["conversation"] = ConversationTools(
                self.config, memory=self.components.get("memory"), voice=self.components.get("voice")
            )
    ...
    async def run(self):
        ...
        if "voice" in self.components:
            self.components["voice"].speak(
                "Привет! Я Елена. Я готова к работе. Можем общаться здесь или ввести браузер для открытия веб-интерфейса."
            )
