...
class TelegramBot:
    ...
    def _get_component_status(self):
        ...
        status = {"memory": False, "voice": False, "vision": False, "tool_executor": False, "memory_count": 0}
    ...
    async def handle_text(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            await asyncio.wait_for(
                context.bot.send_chat_action(chat_id=update.effective_chat.id, action="typing"), timeout=3.0
            )
        except Exception as e:
    ...
    async def _handle_screenshot(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if status["tool_executor"]:
            try:
                result = await self.agent.components["tool_executor"].execute({"type": "take_screenshot", "monitor": 1})
                ...
                if result["success"]:
                    ...
                    await update.message.reply_text(
                        f"❌ Не удалось сделать скриншот: {result.get('error', 'Неизвестная ошибка')}"
                    )
            except Exception as e:
